...
export default function ModalShowUsersLikePost({
      showModalUsersLikePost, 
      setShowModalUsersLikePost, 
      postId
    }: any) {

    const [fetchingUsersLikePost, setFetchingUsersLikePost] = useState<boolean>(false);
    const [allUsers, setAllUsers] = useState<any>([]);

    function closeModal() {
        setShowModalUsersLikePost(false)
    }

    useEffect(() => {
        const getUsersWhoLikeThisComment = async () => {
            setFetchingUsersLikePost(true);
            try {
                ...
                const response = await fetch(`${process.env.REACT_APP_API}/api/v1/instagram/post/like/${postId}`, settings);
              ...
              setFetchingUsersLikePost(false);
            }catch(error) {
                alert(error);
            }
        }

        if(showModalUsersLikePost) {
          getUsersWhoLikeThisComment();
        }
    }, [postId, showModalUsersLikePost]);

  return (
    <>
      <Transition appear show={showModalUsersLikePost} as={Fragment}>
